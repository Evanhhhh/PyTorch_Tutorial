...
class RNNClassifier(torch.nn.Module):
    ...
    def forward(self, input, seq_lengths):
        ...
        # _init_hidden方法，输入参数：batch_size，表示当前批次的样本数量。返回值：形状为 (n_layers * n_directions, batch_size, hidden_size) 的张量。
        ...
        output, hidden = self.gru(gru_input, hidden) # hidden 是 GRU 的最终隐藏状态，形状为 (num_layers * num_directions, batch_size, hidden_size)。
        # 如果 self.n_directions == 2（即双向 RNN），hidden 包含两个方向的隐藏状态：
        # hidden[-1] 是正向最后一层的隐藏状态，形状为 (batch_size, hidden_size)。
        # hidden[-2] 是反向最后一层的隐藏状态，形状为 (batch_size, hidden_size)。
        # 使用 torch.cat 将正向和反向的隐藏状态在特征维度（dim=1）拼接，结果形状为 (batch_size, hidden_size * 2)。
        if self.n_directions == 2:
            hidden_cat = torch.cat([hidden[-1], hidden[-2]], dim=1)
        else:
            hidden_cat = hidden[-1]
        fc_output = self.fc(hidden_cat)  # 通过全连接层 self.fc，将维度从 hidden_size * num_directions 映射到输出类别数
        return fc_output
...
def make_tensors(names, countries):
    ...
    for idx, (seq, seq_len) in enumerate(zip(name_sequences, seq_lengths)):
        # PyTorch 支持张量的切片赋值操作。通过切片操作将 seq 的值批量复制到 seq_tensor 的指定位置，而不是逐个元素赋值，效率更高且代码更简洁。
        seq_tensor[idx, :seq_len] = torch.LongTensor(seq)

    # sort by length to use pack_padded_sequence
    # perm_idx是排序后的数据在原数据中的索引,seq_tensor是排序后的数据,seq_lengths是排序后的数据的长度,countries是排序后的国家
    # 按序列长度从大到小排序，返回排序后的序列长度和对应的原始索引 perm_idx（是tensor类型）
    seq_lengths, perm_idx = seq_lengths.sort(dim=0, descending=True)
    # tensor[index] 会按照 index 中的顺序重新排列 tensor 的数据
    seq_tensor = seq_tensor[perm_idx]
    countries = countries[perm_idx]

    # pack_padded_sequence 要求 lengths 参数必须是 CPU 上的 int64 张量，因此只把 seq_tensor 和 countries 放 GPU
    return create_tensor(seq_tensor), seq_lengths, create_tensor(countries)


# 把名字转换成ASCII码，返回ASCII码值列表和名字的长度
def name2list(name):
    arr = [ord(c) for c in name] # ord(c) 返回字符c的ASCII码值
    return arr, len(arr)
...
def testModel():
    ...
    with torch.no_grad():
        for i, (names, countries) in enumerate(testloader, 1):
            inputs, seq_lengths, target = make_tensors(names, countries)
            output = classifier(inputs, seq_lengths)  # output 的形状是 (batch_size, N_COUNTRY)
            pred = output.max(dim=1, keepdim=True)[1]  # 返回每一行中最大值的那个元素的索引，且keepdim=True，表示保持输出的二维特性
            # view_as(pred)的作用是将target的形状调整为pred的形状，以便进行比较。pred的形状是(batch_size, 1)，而target的形状是(batch_size,)
